...
BIN_NAME="${BIN_NAME:-tunlit-server}"
...
mkdir -p "$PAYLOAD_DIR/client"
cp -r "$ROOT/client/dist" "$PAYLOAD_DIR/client/dist"

(cd "$PAYLOAD_DIR" && npm ci --omit=dev --no-audit --no-fund)
